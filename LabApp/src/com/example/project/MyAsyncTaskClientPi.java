...
/**
 * This class implements the AsyncTask of the Pi client
...
public class MyAsyncTaskClientPi extends AsyncTask<Void, Integer, Boolean> {
	...
	private PiParametersActivity PiActivity;
	private Boolean success=false;
	private Boolean finish=false;
	private DataBaseHelper bd;
	
	/**
     * Constructor that associate the field PiActivity of the class
     * with the PiParametersActivity that has create this AsyncTask
     *
     * @param a the PiParametersActivity that has create this AsyncTask
     */
	public MyAsyncTaskClientPi(PiParametersActivity a) {
		PiActivity=a;
	}
	...
	protected Boolean doInBackground(Void... params) {
		...
		while(!PiActivity.isStopped() && !finish){
			...
			try {
				...
				if(number_recived){
					u=PiActivity.computeU();//Compute (SP-y)*Kp
	...
	*/
    ...
    protected void onProgressUpdate(Integer... progress) {
		if(progress[0].equals(0)){
			...
			success=false;
			if(PiActivity.getConected()){
				PiActivity.setConected(false);
				finish=true;
				PiActivity.changeConected();
			}
		}else if(!recived.equals("Conect")){
			PiActivity.setY(y.intValue());			
			PiActivity.setU(u.intValue());
	...
	*/
	...
	/**
	 * It's the getter of the context of the PiParametersActivity that has called this AsyncTask
	 * @return The context of the PiParametersActivity that has called this AsyncTask
	 */
	...
	/**
	 * It's the setter of the context of the PiParametersActivity that has called this AsyncTask
